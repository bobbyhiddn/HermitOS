...
def configure_system(state: dict, log_cb) -> None:
    ...
    log_cb("Configuring NetworkManager as sole network manager...")
    chroot_run(["systemctl", "enable", "NetworkManager"])

    # Disable conflicting network services so NM is the sole manager
    chroot_run(["systemctl", "disable", "systemd-networkd"])
    chroot_run(["systemctl", "disable", "networking"])
    chroot_run(["systemctl", "mask", "networking"])

    # Write clean /etc/network/interfaces — only loopback
    # This is CRITICAL: Debian's NM ifupdown plugin marks any interface
    # listed here as "unmanaged", which prevents NM from touching ethernet/wifi
    interfaces_content = "# Managed by NetworkManager\nauto lo\niface lo inet loopback\n"
    with open(f"{MOUNT_POINT}/etc/network/interfaces", "w") as f:
        f.write(interfaces_content)

    # Ensure NetworkManager manages all devices (Debian defaults to ifupdown plugin)
    nm_conf_dir = f"{MOUNT_POINT}/etc/NetworkManager/conf.d"
    os.makedirs(nm_conf_dir, exist_ok=True)
    with open(f"{nm_conf_dir}/10-hermit-managed.conf", "w") as f:
