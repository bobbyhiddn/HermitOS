...
class DriveScreen(Screen):
    ...
    def _populate_table(self, devices: list[dict]) -> None:
        ...
        self.query_one("#selection_status", Static).update(
            "Click a drive to select it, then click 'Select Drive'."
        )

    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        """Handle drive selection from the table."""
        ...
        if has_data:
            detail.update(
                f"Selected: {dev_path} ({self._selected_device['size']})\n\n"
        ...
        self.query_one("#selection_status", Static).update(
            f"→ {dev_path} selected. Click 'Select Drive' to confirm."
        )
